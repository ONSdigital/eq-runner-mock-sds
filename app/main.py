...
def get_unit_data(
    ...
    if unit_data := dataset_to_unit_data_map.get(dataset_id).get(identifier):
...
def load_mock_data() -> tuple[list[DatasetMetadata], dict[UUID, dict]]:
    ...
    dataset_to_unit_data_map: dict[UUID, dict] = {}
    ...
    for survey_mock_data_path in MOCK_DATA_ROOT_PATH.rglob("*"):
        ...
        for survey_id in survey_ids:
            for path in survey_mock_data_path.iterdir():
                ...
                dataset_to_unit_data_map[dataset_id] = unit_data

    return dataset_metadata_collection, dataset_to_unit_data_map
